        ...
        #pragma omp parallel num_threads(2)
        {
            if (omp_get_thread_num() == 0) {
                for (int i = 0; i < alphabets_for_master; ++i) {
                    ...
                    if (recv != "") {
                        cout << "Master process on " <<"Jeff's"<< " machine has cracked the password :-)\n";
                        //processorName
            ...
            if (omp_get_thread_num() == 0) {
                for (int i = 0; i < alphabets_for_slaves; ++i) {
                    ...
                    if (recv != "") {
                        ...
                        cout << "Slave " << rank << " process on " << "Jeff's" << " machine has cracked the password :-)\n";
                        break;
                    }
